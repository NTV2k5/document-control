...
type HubDepartmentsProps = {
  departments: any[];
}

export function HubDepartments({ departments }: HubDepartmentsProps) {
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        ...
        <button className="text-sm font-bold text-blue-600 hover:underline">View Directory</button>
      </div>
          ...
          >
            <CardContent className="p-5">
              <div className="mb-5 flex items-start justify-between">
                <div
                  className={`flex h-12 w-12 items-center justify-center rounded-xl bg-${dept.color}-50 text-${dept.color}-500 shadow-sm`}
                >
                  {dept.icon === 'terminal' && <Terminal className="h-6 w-6" strokeWidth={2.5} />}
                  {dept.icon === 'palette' && <Palette className="h-6 w-6" strokeWidth={2.5} />}
                  {dept.icon === 'flask' && <FlaskConical className="h-6 w-6" strokeWidth={2.5} />}
                  {dept.icon === 'sigma' && <Sigma className="h-6 w-6" strokeWidth={2.5} />}
                </div>
                <DropdownMenu>
                  ...
                  >
                    <DropdownMenuItem className="cursor-pointer py-2 text-sm font-medium text-slate-700">
                      <Pencil className="mr-2 h-4 w-4 text-slate-400" /> Rename Dept
                    </DropdownMenuItem>
                    <DropdownMenuItem className="cursor-pointer py-2 text-sm font-medium text-slate-700">
                    ...
                    <DropdownMenuItem className="cursor-pointer py-2 text-sm font-bold text-red-600 focus:bg-red-50 focus:text-red-700">
                      <Trash2 className="mr-2 h-4 w-4" /> Archive Dept
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
              </div>
              <h4 className="mb-1 text-sm font-bold text-slate-900">{dept.name}</h4>
