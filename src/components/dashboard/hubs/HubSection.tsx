    ...
    <div>
      <div className="mb-4 flex items-center justify-between">
        <h3 className="flex items-center gap-2 text-lg font-bold">
          <span className={titleIconColor}>{titleIcon}</span> {title}
        </h3>
            ...
            >
              <CardContent className="p-5">
                <div className="mb-5 flex items-start justify-between">
                  <div
                    className={`flex h-12 w-12 items-center justify-center rounded-xl bg-${item.color}-50 text-${item.color}-500 shadow-sm`}
                  >
                    ...
                    {item.icon === 'rocket' && <Rocket className="h-6 w-6" strokeWidth={2.5} />}
                    {item.icon === 'archive' && <Archive className="h-6 w-6" strokeWidth={2.5} />}
                  </div>
                  <DropdownMenu>
                    ...
                    >
                      <DropdownMenuItem className="cursor-pointer py-2 text-sm font-medium text-slate-700">
                        <Pencil className="mr-2 h-4 w-4 text-slate-400" /> {labels.rename}
                      </DropdownMenuItem>
                      <DropdownMenuItem className="cursor-pointer py-2 text-sm font-medium text-slate-700">
                      ...
                      <DropdownMenuItem className="cursor-pointer py-2 text-sm font-bold text-red-600 focus:bg-red-50 focus:text-red-700">
                        <Trash2 className="mr-2 h-4 w-4" /> {labels.archive}
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
                <h4 className="mb-1 text-sm font-bold text-slate-900">{item.name}</h4>
